        ...
        <div id="js-title-comment-{$oComment->getId()}" class="feed-comment js-title-comment"
            ...
            <ul class="comment-info">
                {if !is_null($oComment->getGuestLogin())}
                    <li class="user-block">{$oComment->getGuestLogin()}</li>
                {else}
                    <li data-alto-role="popover"
                        ...
                        data-cache="true"
                        class="user-block">
                        <img src="{$oUser->getAvatarUrl('small')}" {$oUser->getAvatarImageSizeAttr('small')} {$oUser->getAvatarImageSizeAttr('small')} alt="{$oUser->getDisplayName()}"/>
